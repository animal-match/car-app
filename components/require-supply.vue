<template>
	...
	<view class="my-supply">
		<view class="container">
			...
			<view class="switch-tab">
				<view v-for="(item, index) in buttons" :key="index"
					:class="activeItem===index+1?'active-btn':'default-btn'" @click="switchTab(item.id)">{{item.name}}
				</view>
			</view>
			...
			<view v-if="infos&&infos.length > 0">
				...
				<view v-for="(_item,_index) in infos" :key="_index" class="information-require"
					@click="goDetails(_item.id)">
					<!-- 第一行 标签 标题 按城市-->
					<view class="first-line-other">
						<!-- 左侧 标签和标题 -->
						<view class="left-block-other">
							<u-tag :text="activeItem===1?'供应':'求购'" mode="dark"
								:type="activeItem===1?'primary':'error'" />
					...
					<view class="third-line-other">
						...
						<view class="left-nickname">
							...
							<view :class="pageInfo.pageSort==='myRequireSupply'?'publish-date':'publish-date-normal'">
								{{_item.createtime || '-'}}
							</view>
						</view>
						<!-- 右侧 时间 -->
						<view class="operation-btn" v-if="pageInfo.pageSort==='myRequireSupply'">
							<u-button @click="goDetails(_item.id)" type="primary" size="mini" class="check">查看
							</u-button>
							<u-button @click="deleteItem(_item.id)" type="error" size="mini" class="delete">删除
							</u-button>
						</view>
					</view>
				</view>
		...
		<u-modal v-model="show" content="确定要删除吗？" width="50%" @confirm="deleteConfirm(userid)"
